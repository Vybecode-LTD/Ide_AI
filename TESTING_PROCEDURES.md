...
> ## ⚠️ PROJECT OVERRIDE (Ide/AI) — scope + the no-preview rule
>
> **Active stacks for Ide/AI: only #2 below — Python/FastAPI/PostgreSQL backend + React/Vite
> frontend.** The C#/Avalonia sections (stack #1) do **not** apply and should be skipped.
>
> **No preview verification — [Critical Rule #8](CLAUDE.md) wins.** This project does **not** start
> dev servers, launch the app, take screenshots, or run browser/Playwright E2E suites to verify
> changes — the user tests manually. So the instructions below to "verify the Vite dev server
> starts," serve built assets in a browser, or run E2E/packaged-artifact launches are
> **superseded**. Automated verification on Ide/AI is: `pytest` (backend), `npx.cmd vitest run`
> (frontend), `npx.cmd tsc -b --noEmit` (types) — hand live/browser testing to the user. Everything
> else here (test-first, regression test per bug fix, coverage discipline, the evidence ledger,
> `@DEBUG_PROTOCOL.md`) applies in full.
