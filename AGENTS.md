...
> This file is the single source of truth for Codex sessions working on this project.
...
| Layer | Stack |
|-------|-------|
| Backend | Python 3.12, FastAPI, SQLAlchemy 2.0 (async), Alembic, PostgreSQL, Anthropic Codex API (Codex-sonnet-4-6) |
...
- Platform options: Bubble, Webflow, FlutterFlow, Bolt, Lovable, Codex, Cursor, Replit, n8n, Custom
...
- AI model: Anthropic Codex (Codex-sonnet-4-6), configurable via CLAUDE_MODEL env var
...
- 7 platform templates (Jinja2) — each written in that platform's expected format/terminology:
  ...
  - Codex: database schemas, API endpoints, phased implementation plan
...
- On session start, read this AGENTS.md file first.
